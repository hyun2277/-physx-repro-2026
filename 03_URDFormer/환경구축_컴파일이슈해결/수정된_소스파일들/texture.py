...
# 2026-09-16 수정: 원래 이 두 파이프라인을 모듈을 import하는 순간(무조건) 불러왔는데, 이러면
# demo.py를 --texture 없이 실행해도(README상 "선택사항"이라고 명시된 사용법) gated 모델
# (HuggingFace 로그인+라이선스 동의 필요) 다운로드가 강제로 시도돼 실패했음. load_texture/
# load_kitchen_texture가 실제로 호출될 때만 지연 로드하도록 변경 — 계산 로직은 무수정,
# "텍스처를 실제로 쓸 때만 불러온다"는 README의 설명대로 동작하게 만드는 수정임.
depth2img_pipe = None
in_paint_pipe = None


def _ensure_texture_pipelines_loaded():
    global depth2img_pipe, in_paint_pipe
    if depth2img_pipe is not None and in_paint_pipe is not None:
        return
...
def load_kitchen_texture(image, test_name, object_id, bboxes):
    _ensure_texture_pipelines_loaded()
    # load the image
