...
def traj_camera(view_matrix):
    ...
    light_pos = [3, 1.5, 5]
    # 2026-09-16 수정: 이 함수는 --headless(p.DIRECT 연결) 모드에서만 호출되는데, 원래 코드는
    # 실제 창의 OpenGL 컨텍스트가 있어야 하는 ER_BULLET_HARDWARE_OPENGL 렌더러를 그대로 써서
    # DIRECT 연결에서는 1차원 배열이 반환되며 깨졌음(색 배열을 3차원으로 인덱싱하다 IndexError).
    # headless/DIRECT 연결에 맞는 CPU 소프트웨어 렌더러(ER_TINY_RENDERER)로 교체 — 계산 로직·
    # 카메라 파라미터는 무수정, 렌더러 종류만 headless 환경에 맞게 바꾼 것.
    _, _, color, depth, segm= p.getCameraImage(512, 512, view_matrix, projection_matrix, light_pos, shadow=1, flags=p.ER_SEGMENTATION_MASK_OBJECT_AND_LINKINDEX, renderer=p.ER_TINY_RENDERER)
    # 2026-09-16 수정: 이 환경의 pybullet 빌드(Windows용 사전빌드 파일이 없어 소스 컴파일)는
    # color를 (512,512,4) 모양이 아니라 평탄화된 1차원 튜플(길이 512*512*4)로 반환함
    # (다른 환경의 pybullet은 numpy 통합이 켜져 있어 자동으로 모양이 잡히는데, 이 빌드는 아님을
    # 직접 확인함). reshape 후 원래 코드와 동일하게 RGB 3채널만 추출.
    color = np.reshape(color, (512, 512, 4))
    # 2026-09-16 수정: 위와 같은 이유로 색상값이 int32로 나와서 PIL.Image.fromarray가 처리 못 함
    # (uint8만 지원) — 값 범위(0~255)는 그대로 유지한 채 타입만 uint8로 캐스팅.
    rgb = np.array(color)[:,:, :3].astype(np.uint8)
    return rgb
