...
RUN="$ROOT/logs/inference-29354-run/$RUN_ID"
OUT="$ROOT/staging/inference-29354-$RUN_ID"
mkdir -p "$RUN" "$OUT"
...
printf '%q ' env CUDA_VISIBLE_DEVICES=1 CUDA_HOME="$CUDA_HOME" CUDACXX="$CUDACXX" CC="$CC" CXX="$CXX" CUDAHOSTCXX="$CUDAHOSTCXX" NVCC_CCBIN="$NVCC_CCBIN" PATH="$PATH" HOME="$SRC" XDG_CACHE_HOME="$ROOT/cache" "$PY" example.py --condpath "$INPUT" --savepath "$OUT" > "$RUN/example.command.txt"
date -u +%Y-%m-%dT%H:%M:%SZ > "$RUN/start_utc.txt"
set +e
(cd "$SRC" && PYTHONUNBUFFERED=1 env CUDA_VISIBLE_DEVICES=1 CUDA_HOME="$CUDA_HOME" CUDACXX="$CUDACXX" CC="$CC" CXX="$CXX" CUDAHOSTCXX="$CUDAHOSTCXX" NVCC_CCBIN="$NVCC_CCBIN" PATH="$PATH" HOME="$SRC" XDG_CACHE_HOME="$ROOT/cache" "$PY" example.py --condpath "$INPUT" --savepath "$OUT") > "$RUN/example.stdout.log" 2> "$RUN/example.stderr.log" &
example_pid=$!
(
